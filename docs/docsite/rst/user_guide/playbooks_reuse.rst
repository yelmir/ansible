...
Roles allow more than just tasks to be packaged together and can include variables, handlers, or even modules and other plugins. Unlike playbooks and tasks files, roles can also be uploaded and shared via Ansible Galaxy.
...
In Ansible 2.0, the concept of *dynamic* includes was introduced. Due to some limitations with making all includes dynamic in this way, the ability to force includes to be *static* was introduced in Ansible 2.1. Because the *include* task became overloaded to encompass both static and dynamic syntaxes, and because the default behavior of an ``include`` could change based on its context, Ansible 2.4 introduces the concept of ``include`` vs. ``import``.

If you use any ``import*`` (``import_playbook``, ``import_tasks``, etc.), it will be *static*.
If you use any ``include*`` (``include_tasks``, ``include_role``, etc.), it will be *dynamic*.
...
The new ``import_role`` is a more versatile version of ``roles``, allowing you to set the tasks file entry point as well as control variable behavior. If you simply want to leverage roles in your playbook without the need for more sophistocated options, ``roles`` is the best option.

Here is a description of the default behavior of the now-deprecatped ``include`` in various contexts and its equivalent with the new and explicit ``import*`` and ``include*`` syntax.

+--------------+--------------------+-------------------------------+----------------------------------+
| Context      | Behavior           | Old Syntax                    | New Syntax                       |
+==============+====================+===============================+==================================+
| Play         | Static             | ``include: site.yml``         | ``import_playbook``              |
+--------------+--------------------+-------------------------------+----------------------------------+
| Role         | Static             | ``roles``                     | ``import_role``                  |
|              |                    |                               | ``roles``                        |
+--------------+--------------------+-------------------------------+----------------------------------+
| Task         | Static             | ``include: tasks.yml``        | ``import_tasks``                 |
+--------------+--------------------+-------------------------------+----------------------------------+
| Task         | Dynamic            | ``include: tasks.yml`` with   | ``include_tasks``                |
|              |                    | ``static: no``                |                                  |
+--------------+--------------------+-------------------------------+----------------------------------+
| Role         | Dynamic            | None                          | ``include_role``                 |
+--------------+--------------------+-------------------------------+----------------------------------+
...
.. note::
    ...
    - The preferred method for using roles in playbooks in now ``import_role`` or ``include_role`` in the ``tasks`` section of a playbook. This gives the playbook author total control of the order in which roles are executed without having to relay on ``pre_tasks`` and ``post_tasks``.

Tradeoffs and Pitfalls Between Includes and Imports
```````````````````````````````````````````````````

There are pros and cons to both ``import*`` and ``include*`` to take into consideration when deciding which to use. For most use cases, use ``import*``. Only use ``include*`` when the specific behavior of dynamicly including tasks or roles provides.
